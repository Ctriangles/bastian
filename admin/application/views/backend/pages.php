                ...
                <table id="datatable" class="table table-striped table-bordered dt-responsive" style="border-collapse: collapse; border-spacing: 0; width: 100%;">
                    <thead>
                        <tr>
                            ...
                            <th>Page URL</th>
                            <th>Last Edit</th>
                            <?php if(array_search('pages_edit',$this->role_list) !== false || array_search('pages_delete',$this->role_list) !== false) { ?>
                            <th>Action</th>
                            <?php } ?>
                        </tr>
                    </thead>
                    <tbody>
                        ...
                        <tr>
                            ...
                            <td><?=$AllData->post_slug?></td>
                            <td><?=date('d M, Y (h:i A) ', strtotime($AllData->edit_date));?></td>
                            <?php if(array_search('pages_edit',$this->role_list) !== false || array_search('pages_delete',$this->role_list) !== false) { ?>
                            <td>
                                <?php if(array_search('pages_edit',$this->role_list) !== false) { ?>
                                <a class="btn btn-outline-success waves-effect waves-light" href="<?=site_url('backend/EditPage/'.$AllData->id.'/')?>"><i class="uil-edit"></i></a>
                                <?php } if(array_search('pages_delete',$this->role_list) !== false) { ?>
...
<script>
    function delFunction(pageid) {
        $.ajax({
            type: "POST",
            url: '<?=site_url('post_controller/PostTrash')?>',
            data : {"id" : pageid},
            beforeSend: function(){
    ...
    function revFunction(pageid) {
        $.ajax({
            type: "POST",
            url: '<?=site_url('post_controller/PostTrashReverse')?>',
            data : {"id" : pageid},
            beforeSend: function(){
                $(".card").append('<div class="preloaderremove"><div class="preloader"><div class="spinner"></div></div></div>');
            },
    ...
    function delpFunction(pageid) {
        $.ajax({
            type: "POST",
            url: '<?=site_url('post_controller/PostTrashPerma')?>',
            data : {"id" : pageid},
            beforeSend: function(){
                $(".card").append('<div class="preloaderremove"><div class="preloader"><div class="spinner"></div></div></div>');
            },
