            ...
            <div class="card-body">
                <table id="datatable" class="table table-striped table-bordered dt-responsive nowrap" style="border-collapse: collapse; border-spacing: 0; width: 100%;">
                    ...
                    <tbody>
                        ...
                        <tr>
                            <td><?=$i?></td>
                            <td><?=$AllData->first_name.' '.$AllData->last_name?></td>
                            <td><?=$AllData->user_email?></td>
                            <td><?='('.$AllData->country_code.') '.$AllData->phone?></td>
                            <td><?=(!empty($AllData->user_type))?$AllData->role_name:''?><?=($AllData->user_for == 'front')?'Front User':''?></td>
                            <td><?=date('d M, Y (h:i A) ', strtotime($AllData->edit_date));?></td>
                            <?php if(array_search('manage_roles_edit',$this->role_list) !== false || array_search('manage_roles_delete',$this->role_list) !== false) { ?>
                            <td>
                                <?php if(array_search('manage_roles_edit',$this->role_list) !== false) { ?>
                                <a class="btn btn-outline-success waves-effect waves-light" href="<?=site_url('backend/EditUsers/'.$AllData->id.'/')?>"><i class="uil-edit"></i></a>
...
<script>
    function delFunction(pageid) {
        $.ajax({
            type: "POST",
            url: '<?=site_url('user_controller/UsersTrash')?>',
            data : {"id" : pageid},
            beforeSend: function(){
    ...
    function revFunction(pageid) {
        $.ajax({
            type: "POST",
            url: '<?=site_url('user_controller/UsersTrashReverse')?>',
            data : {"id" : pageid},
            beforeSend: function(){
                $(".card").append('<div class="preloaderremove"><div class="preloader"><div class="spinner"></div></div></div>');
            },
    ...
    function delpFunction(pageid) {
        $.ajax({
            type: "POST",
            url: '<?=site_url('user_controller/UsersTrashPerma')?>',
            data : {"id" : pageid},
            beforeSend: function(){
                $(".card").append('<div class="preloaderremove"><div class="preloader"><div class="spinner"></div></div></div>');
            },
